...
class editPost : AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (position != -1) {
            MyPostsProvider.myPostsList[position].Title?.let { Log.e("Ayuda 1:", it) }
            MyPostsProvider.myPostsList[position].description?.let { Log.e("Ayuda 2: ", it) }
            val textTile = findViewById<EditText>(R.id.etTitulo)
            val textDescrip = findViewById<EditText>(R.id.etDescripcion)
            textTile.setText(MyPostsProvider.myPostsList[position].Title)
            textDescrip.setText(MyPostsProvider.myPostsList[position].description)
            autoCompleteTextView.setText(MyPostsProvider.myPostsList[position].countryName, false)
            countryId = MyPostsProvider.myPostsList[position].idCountry
            idpost = MyPostsProvider.myPostsList[position].idPost
            MyPostsProvider.myPostsList[position].PostImg?.forEach { elemento->
        ...
        val btndelete: ImageView = findViewById(R.id.imgPictureDelete)
        ...
        val btnCancelar : Button = findViewById(R.id.btnDraft)
